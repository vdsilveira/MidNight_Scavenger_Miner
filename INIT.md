...
### 4) Instalar dependências (scavenger-api)
```bash
cd /home/norman/codigos/Midnight_Miner/scavenger-api
npm i
```

### 5) Rodar a API (NestJS)
```bash
npm run start:dev
```
...
### 6) Registro e mineração automática
...
### 7) Teste de mineração com dificuldade reduzida
...
### 8) (Opcional) Habilitar binário nativo do AshMaize
...
# 4) (Opcional) Rodar teste de mineração fácil
npx ts-node -T test-mining-easy.ts
```
